...
Записываем значение в локальную память только при CLK 0->1 и COUNTER==2.

Записываем мы мантиссу, к котрой приписали 10 нулей.

Далее считаем корень итерационно, код повторяет теор часть.

**testbanch**

В sqrt2_log.txt записываю номер теста, входное число, далее все промежуточные значения, которые выводятся. И в конце итоговый результат после RESULT=1. И ожидаемое значение.

**remarke**

почему то пока что максимальное денорм число в verilog обрабатывается неправильно, в logisim ответ верный
