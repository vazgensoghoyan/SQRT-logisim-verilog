...
Тестбенч sqrt2_tb проверяет корректность работы модуля sqrt2.

Каждую решетку меняется значение CLK, то есть каждый такт длится одну "единицу времени" (не знаю, как правильно назвать) и еще одну такт не начинается (CLK = 0).

Подает на IO_DATA sqrt2 на 1 такте входные данные, а между 1 и 2 тактами устанавливает в высокоимпеданс. 

Начиная со второго такта вплоть (включая) до момента установления RESULT=1 выводит промежуточные значения на выходах модуля. Делаю это на negedge CLK, то есть такт полностью закончен и между этим тактом и следующим вывожу эти логи.

Когда этот цикл закончен, то есть RESULT=1 и модуль должен быть зафиксирован (внутреннее состояние), выводим либо PASSED, либо FAILED с ожидаемым значением.

Также тестбенч в конце для удобства выводит либо ALL PASSED, либо FAILED со списком непройденных тестов.
